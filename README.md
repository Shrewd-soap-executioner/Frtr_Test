...
1. Откройте веб-интерфейс по адресу `http://localhost:3000` и перейдите на страницу регистрации.
2. **Ввод email:** Введите почту в стандартном формате (например, `user@example.com`). Обратите внимание, что встроенная валидация не пропустит некорректный формат (без `@` и домена).
3. **Важно:** Указывайте **реальный email-адрес**, к которому у вас есть доступ. Сразу после регистрации сервер отправит на эту почту письмо с вашим первым ключом активации.
4. Придумайте пароль (пароли в обоих полях должны совпадать) и нажмите кнопку регистрации.
5. Зайдите на свою почту, скопируйте полученный ключ и вставьте его в десктопное приложение для получения прокси-сервера.
